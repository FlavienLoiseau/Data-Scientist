def program(my_choice)
  ...
  when 4
    puts journalists.count { |a| a=~/^@[A-Z]/}
  when 5
    puts journalists.sort_by { |a| a.downcase}
  when 6
    puts journalists.sort_by { |a| a.length}
  when 7
    puts journalists.index("@epenser")
  when 8
    max_string = journalists.max { |a, b| a.length<=>b.length}
    i = 1
    j = max_string.length
    while i<=j
      number = journalists.count { |a| a.length==i}
      puts "Il y a #{number} noms à #{i} lettres"
      i+=1
    end 
  else
    puts "relance le programme et entre un chiffre entre 1 et 9"
  end
end

def my_choice
  puts "1: Nombre de handle dans la liste"
  puts "2: Handle le plus court de la liste"
  puts "3: Nombre de handle contenant 5 caractères"
  puts "4: Nombre de handle commençant par une majuscule"
  puts "5: Liste triée par ordre alphabétique"
  puts "6: Liste triée par taille de handle"
  puts "7: Position du handle @epenser"
  puts "8: Nombre de handle par nombre de lettres"
